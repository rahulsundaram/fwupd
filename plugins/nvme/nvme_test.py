...
A: power/pm_qos_latency_tolerance_us=100000
...
A: power/control=on
A: power/runtime_active_time=17184949
...
L: iommu=../0000:00:00.2/iommu/ivhd0
L: iommu_group=../../../kernel/iommu_groups/3
A: irq=39
A: local_cpulist=0-11
A: local_cpus=0fff
A: max_link_speed=16.0 GT/s PCIe
...
A: power/control=auto
A: power/runtime_active_time=17184970
A: power/runtime_status=active
A: power/runtime_suspended_time=0
A: power/wakeup=disabled
A: power/wakeup_abort_count=
...
"""
    ...
    def test_nvme_device(self):
        def handle_ioctl(handler, client):
            ...
            if client.get_request() != 3225964097:
                return False
            arg = client.get_arg()
            ...
            if in_data.cdw11 != 0x00:
                return False
            print(f"address: {in_data.addr}")

            # need to copy CNS data to that address...
            # update the data
            # data.update(0, bytearray(in_data))
